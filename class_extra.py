...
class extra(pd.DataFrame):
        ...
        for col in cols:
            try:
                ...
                for kind in ['bar', 'pie'][0:count_.shape[0]-1]:
                    ...
                    if count_.shape[0]>8:
                        plt.figure()
                        count_[1:5].plot(kind=kind, title=u"%s - valeurs 2 à 4 "%(col))
                        plt.figure()                    
                        count_[5:].plot(kind=kind, title=u"%s - valeurs 4 et plus "%(col))
    ...
    def timeline2(df, path, col_date, debug=False):    
        ...
        for col in cat_col:
            ...
            categories = test2[col].value_counts().keys()
            ...
            for cat in categories : 
                ...
                try:
                    ...
                    """    
                    title_ = u"TABLE_[%s]_COLONNE_[%s]_CATEGORIE_[%s]_nombre_par_jour_par_date_de_[%s]"%(nom_table_utf8, col_utf8, cat_utf8, col_date_utf8)
                    ...
                    savedir = u"./../images/table_%s/col_%s/"%(nom_table_utf8.replace("/", "").replace(" ", "_"), col_utf8.replace("/", "").replace(" ", "_"))
                    print savedir
                    try: 
                        os.makedirs(savedir)
                    except OSError:
                        if not os.path.isdir(savedir):
                            raise
                    plt.savefig(savedir+u"%s.jpg"%title_)
                    plt.show()
